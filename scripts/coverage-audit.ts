...
function main(): void {
  ...
  const md = [
    "# SDK Coverage Audit",
    "",
    `Generated by \`pnpm audit:coverage\`. Total LinearClient members: **${members.length}**.`,
    "",
  ...
  if (!existsSync(snapshotPath)) {
    console.error("No coverage snapshot found. Run `pnpm audit:coverage --update` to create it.");
    process.exit(1);
  }
  ...
  if (added.length || removed.length || changed.length) {
    ...
    console.error("Coverage drift detected. Re-baseline with `pnpm audit:coverage --update`.");
    process.exit(1);
  }
